(function () {
  ...
  function renderSongs() {
    ...
    filtered.forEach((song) => {
      ...
      button.setAttribute("aria-label", `Ouvrir le chant ${song.titre}`);
      button.innerHTML = `
        <div class="flex gap-4">
          <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-800" aria-hidden="true">♪</span>
  ...
  function renderSongDetail(songId) {
    ...
    dom.detail.innerHTML = `
      ...
      <section class="mt-7">
        ...
        <div class="lyric-box mt-4 rounded-[1.4rem] border border-slate-200 bg-slate-50 p-5 text-slate-700">
          ${song.paroles ? AprogosHelpers.escapeHtml(song.paroles) : "Les paroles de ce chant ne sont pas encore disponibles."}
        </div>
      </section>`;
  }
